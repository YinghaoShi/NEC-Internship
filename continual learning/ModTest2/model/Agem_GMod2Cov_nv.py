...
def project2cone2(gradient, memories, current_Tid, margin=0.5,grads_groups=None,relatedness=None,args=None):
    ...
    """    
    #if current_Tid==1:
    #    print("grads_groups shape:{}".format(len(grads_groups[1])))
    gradient_np = gradient.cpu().contiguous().view(-1).double().numpy()
    #print("relatedness:{}".format(relatedness[current_Tid]))
    h = transform_relatedness(relatedness[current_Tid],args)
    
    #print("h content:{}".format(h))

    for tid in range(current_Tid):     
        #print("gredient:{}".format(len(grads_groups[tid])))   
        for pi, p in enumerate(grads_groups[tid]):
            #if pi!=0:
            #print("p shape:{}".format(p.shape))
            pp = p if ((tid==0) & (pi==0)) else np.concatenate((pp, p), axis=0)        
    #print("pp 0 number:{}".format(np.sum(pp==0)))
    ...
    """ #assign groups into tasks(relatedness inverted)
    for i in range(current_Tid):
        if i == 0:
            #print("temp:{}".format((1/(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]))/\
            #np.sum(1/(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]))))
            #print("current_Tid:{}".format(current_Tid))
            #print("h content:{}".format(h))
            #print("portion:{}".format(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]))
            temp = np.dot((1/(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]))/\
            np.sum(1/(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)])),\
            ...
            #print("current_Tid:{}".format(current_Tid))
            #print("portion:{}".format(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]))
            temp_ = np.dot((1/(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]))/\
            np.sum(1/(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)])),\
            pp[i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups),:])
    ...
    for i in range(current_Tid):
        if i == 0:
            temp = np.dot(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]/\
            np.sum(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]),\
            pp[i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups),:])
        else:
            temp_ = np.dot(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]/\
            np.sum(np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]),\
            pp[i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups),:])
            temp = np.vstack((temp,temp_)) """

    #only consider the last 5 groups(last layer) and sum up them
    for i in range(current_Tid):
        if i == 0:
            #lastfive_final = [[0]]
            #lastfive = np.expand_dims(h,axis=0)[:,i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups)]
            lastfive = 1/(np.expand_dims(h,axis=0)[:,i*(args.n_layers*args.num_groups):(i+1)*(args.n_layers*args.num_groups)])
            #print("lastfive original:{}".format(lastfive))
            lastfive = (lastfive-np.mean(lastfive)+1).tolist()
            lastfive[0].insert(0,1)
            #print("lastfive mean moved:{}".format(lastfive))
            #print("previous gradient:{}".format(pp[i*(1+args.n_layers*args.num_groups):(i+1)*(1+args.n_layers*args.num_groups),:][2,:]))
    ...
    #print("memories shape:{}".format(torch.reshape(memories,(-1,89610)).numpy()))
    #print("result:{}".format(np.sum(np.abs(temp-torch.reshape(memories,(-1,89610)).numpy()))))
    ...
    #print("updated gradient:{}".format(x))  
    ...
    def observe_learn(self, x, t, y):
        ...
        # for the other tasks: accumulate the input data
            ...
            if len(self.cov_list)<= t:              
                ...
                #print("relatedness list:{}".format(self.relatedness_list))
        ...
        #print("relatedness:{}".format(self.relatedness_list))
