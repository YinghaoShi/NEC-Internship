...
def find_groups(args,W_ij_ab,W_ij_bc,debug=False):
    ...
    #print("W_ij_ab:{}".format(W_ij_ab))
    #print("W_ij_bc:{}".format(W_ij_bc))
    ...
    for t in range(args.iterations):
        ...
        #Eq 9
        #q_kc_num = np.array([Pi[c]*np.prod(tau_b[:,c:(c+1)]**B.transpose(),axis=0)*np.prod(tau_a[:,c:(c+1)]**A,axis=0) for c in range(args.c)])
        ...
        #new version
        q_kc=np.array(np.exp(log_q_kc)/np.exp(log_q_kc).sum(axis=0,keepdims=True)).transpose()
        
        #experiment call
        #python main.py --model ewc_GMod2Cov --n_tasks 10 --lr 0.01 --n_memories 10 --memory_strength 10 --n_layers 2 --n_hiddens 100 
        # --data_path data/ --save_path results/ --batch_size 1 --log_every 100 --samples_per_task 1000 --cuda no --seed 3 --beta 0.03 --gamma 1.0 
        # --memories 200 --replay_batch_size 10 --batches_per_example 10 --forgetting_mode False --forgetting_task_ids 0,5 
        # --forgetting_resee_size 100 --sign_attacked -1.0 --num_groups 20 --cov_recompute_every 20 --create_random_groups False 
        # --divergence von_Neumann --if_output_cov False --cov_first_task_buffer 100 --data_file fashion_mnist_permutations_reduced.pt 
        # --ewc_reverse False --create_group_per_unit False
